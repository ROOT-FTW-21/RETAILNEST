...
public class AuthController {
	...
	public ResponseEntity<AuthResponseDTO> getUserById(@PathVariable Long id, @RequestHeader("X-UserId") String userid,
			@RequestHeader("X-Roles") String roles) {
		return ResponseEntity.ok(authApplicationService.getUserById(id, userid, roles));
	}
	...
	public ResponseEntity<List<AuthResponseDTO>> findAllUsers(@RequestHeader("X-UserId") String currentUserId,
			@RequestHeader("X-Roles") String userRolesString) {

		return ResponseEntity.ok().body(authApplicationService.getAllUsers(currentUserId, userRolesString));
	}
}
